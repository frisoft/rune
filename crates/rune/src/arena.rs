...
#[non_exhaustive]
pub(crate) struct ArenaWriteSliceOutOfBounds {
    pub(crate) index: usize,
}

#[non_exhaustive]
pub(crate) struct ArenaAllocError {
...
/// Deallocation glue.
struct Dealloc(ptr::NonNull<()>, unsafe fn(*mut ()));

/// An arena allocator.
pub struct Arena {
    start: Cell<*mut u8>,
    end: Cell<*mut u8>,
    // Deallocators to call once dropped.
    dealloc: RefCell<Vec<Dealloc>>,
    // Chunks of memory.
    ...
    pub(crate) fn alloc<T>(&self, object: T) -> Result<&mut T, ArenaAllocError> {
        ...
        unsafe {
            ...
            if mem::needs_drop::<T>() {
                self.dealloc
                    .borrow_mut()
                    .push(Dealloc(ptr.cast(), drop_glue::<T>));
            }

            return Ok(ptr.as_mut());
        }

        // Helper glue to drop an arena-allocated object.
        unsafe fn drop_glue<T>(this: *mut ()) {
...
// This destructor has to live on the Arena to ensure that we call destructors
// *before* the chunks they point into are being dropped.
impl Drop for Arena {
    fn drop(&mut self) {
        for Dealloc(ptr, drop) in self.dealloc.borrow_mut().drain(..) {
            // SAFETY: Dealloc is constructed internally and any invariants present
            // are checked. The arena will ensure that no borrows are active as
            // elements are being dropped.
            unsafe {
                drop(ptr.cast().as_ptr());
            }
        }
    }
}
